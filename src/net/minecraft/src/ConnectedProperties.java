...
public class ConnectedProperties {
    ...
    public boolean matchesBlock(BlockStateBase blockState) {
        ...
        if (this.matchBlocks != null) {
            ...
            for (metadata = 0; metadata < this.matchBlocks.length; ++metadata) {
                MatchBlock i = this.matchBlocks[metadata];

                if (i.matches(blockState)) {
                    matchMetadata = true;
                    break;
                }
            }
        ...
        if (this.metadatas != null) {
            matchMetadata = false;
            metadata = blockState.getMetadata();

            for (int var6 = 0; var6 < this.metadatas.length; ++var6) {
                int md = this.metadatas[var6];
