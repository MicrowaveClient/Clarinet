...
public class ConnectedTextures {
                ...
                if (blockId >= 0 && blockId < tileProperties.length) {
                    ...
                    if (cps != null) {
                        ...
                        for (i = 0; i < cps.length; ++i) {
                            ...
                            if (cp != null && cp.matchesBlock(blockStateBase)) {
                                newIcon = getConnectedTexture(cp, blockAccess, blockState, blockPos, side, icon, metadata, renderEnv);
                ...
                if (blockId >= 0 && blockId < blockProperties.length) {
                    ...
                    if (cps != null) {
                        ...
                        for (i = 0; i < cps.length; ++i) {
                            ...
                            if (cp != null && cp.matchesIcon(icon) && cp.matchesBlock(blockStateBase)) {
                                newIcon = getConnectedTexture(cp, blockAccess, blockState, blockPos, side, icon, metadata, renderEnv);
    ...
    public static int getSide(EnumFacing facing) {
        if (facing == null) {
            ...
            switch (ConnectedTextures.NamelessClass152629133.$SwitchMap$net$minecraft$util$EnumFacing[facing.ordinal()]) {
                case 1:
                    return 0;
        ...
        if (y >= cp.minHeight && y <= cp.maxHeight) {
            ...
            if (cp.biomes != null) {
                ...
                for (int block = 0; block < cp.biomes.length; ++block) {
                    BiomeGenBase sideCheck = cp.biomes[block];

                    if (vertAxis == sideCheck) {
                        metadataCheck = true;
                        break;
                    }
                }
            ...
            if (var15 instanceof BlockRotatedPillar) {
                var13 = getWoodAxis(side, metadata);
                var14 = metadata & 3;
            }

            if (var15 instanceof BlockQuartz) {
                var13 = getQuartzAxis(side, metadata);
            ...
            if (side >= 0 && cp.faces != 63) {
                ...
                if (var13 != 0) {
                    var16 = fixSideByAxis(side, var13);
                }

                if ((1 << var16 & cp.faces) == 0) {
                    return null;
                }
            }
            ...
            switch (cp.method) {
                ...
                case 2:
                    return getConnectedTextureHorizontal(cp, blockAccess, blockState, blockPos, var13, side, icon, metadata);

                case 3:
                    return getConnectedTextureTop(cp, blockAccess, blockState, blockPos, var13, side, icon, metadata);
                ...
                case 6:
                    return getConnectedTextureVertical(cp, blockAccess, blockState, blockPos, var13, side, icon, metadata);

                case 7:
                    return getConnectedTextureFixed(cp);

                case 8:
                    return getConnectedTextureHorizontalVertical(cp, blockAccess, blockState, blockPos, var13, side, icon, metadata);

                case 9:
                    return getConnectedTextureVerticalHorizontal(cp, blockAccess, blockState, blockPos, var13, side, icon, metadata);
    ...
    static class NamelessClass152629133 {
