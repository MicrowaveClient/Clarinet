    ...
    public static List getFaceQuads(IBlockAccess blockAccess, Block block, BlockPos blockPos, EnumFacing facing, List quads) {
        if (facing != EnumFacing.UP && facing != EnumFacing.DOWN) {
            if (block instanceof BlockMycelium) {
                return Config.isBetterGrassFancy() ? (getBlockAt(blockPos.down(), facing, blockAccess) == Blocks.mycelium ? modelCubeMycelium.getFaceQuads(facing) : quads) : modelCubeMycelium.getFaceQuads(facing);
            } else {
