...
public class PlayerControllerMP {
    ...
    public boolean onPlayerDamageBlock(BlockPos posBlock, EnumFacing directionFacing) {
        ...
        DamageBlockSubject damageBlockSubject = new DamageBlockSubject(posBlock.getX(), posBlock.getY(), posBlock.getZ(),
                blockHitDelay, curBlockDamageMP, directionFacing);
        Glade.getInstance().getSubjectManager().dispatch(damageBlockSubject);

        this.syncCurrentPlayItem();

        if (blockHitDelay != damageBlockSubject.getBlockHitDelay()) {
            blockHitDelay = damageBlockSubject.getBlockHitDelay();
            shouldReplaceHitDelay = true;
        }
        this.curBlockDamageMP = damageBlockSubject.getCurBlockDamageMP();
        if (shouldReplaceHitDelay)
            blockHitDelay = damageBlockSubject.getBlockHitDelay();

        if (blockHitDelay > 0) {
            blockHitDelay = blockHitDelay - 1;
            return true;
