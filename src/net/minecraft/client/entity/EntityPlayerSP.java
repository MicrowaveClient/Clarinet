...
import me.nuf.glade.subjects.PushOutOfBlocksSubject;
...
public class EntityPlayerSP extends AbstractClientPlayer {
    ...
    public void onUpdateWalkingPlayer() {
        MotionUpdateSubject motionUpdateSubject = new MotionUpdateSubject(MotionUpdateSubject.Time.PRE, rotationYaw, rotationPitch, posX, this.getEntityBoundingBox().minY, posZ, onGround);
        Glade.getInstance().getSubjectManager().dispatch(motionUpdateSubject);

        if (motionUpdateSubject.isCancelled())
            return;

        posX = motionUpdateSubject.getX();
        double y = motionUpdateSubject.getY();
        posZ = motionUpdateSubject.getZ();
        onGround = motionUpdateSubject.isOnGround();

        float yaw = motionUpdateSubject.getRotationYaw();
        float pitch = motionUpdateSubject.getRotationPitch();
        ...
        if (this.isCurrentViewEntity()) {
            ...
            double var5 = y - this.lastReportedPosY;
            double var7 = this.posZ - this.lastReportedPosZ;
            double var9 = (double) (yaw - this.lastReportedYaw);
            double var11 = (double) (pitch - this.lastReportedPitch);
        ...
        rotationYaw = motionUpdateSubject.getOldRotationYaw();
        rotationPitch = motionUpdateSubject.getOldRotationPitch();

        MotionUpdateSubject motionUpdateSubject1 = new MotionUpdateSubject(MotionUpdateSubject.Time.POST);
        Glade.getInstance().getSubjectManager().dispatch(motionUpdateSubject1);
    }
    ...
    protected boolean pushOutOfBlocks(double x, double y, double z) {
        PushOutOfBlocksSubject pushOutOfBlocksSubject = new PushOutOfBlocksSubject();
        Glade.getInstance().getSubjectManager().dispatch(pushOutOfBlocksSubject);
        if (pushOutOfBlocksSubject.isCancelled())
            return false;
        if (this.noClip) {
            return false;
        } else {
    ...
    public void onLivingUpdate() {
        ...
        if (this.isUsingItem() && !this.isRiding()) {
            ItemUseSubject itemUseSubject = new ItemUseSubject(0.2F);
            Glade.getInstance().getSubjectManager().dispatch(itemUseSubject);
            this.movementInput.moveStrafe *= itemUseSubject.getSpeed();
            this.movementInput.moveForward *= itemUseSubject.getSpeed();
