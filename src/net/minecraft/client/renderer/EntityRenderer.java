...
public class EntityRenderer implements IResourceManagerReloadListener {
    ...
    private void hurtCameraEffect(float partialTicks) {
        HurtcamSubject hurtcamSubject = new HurtcamSubject();
        Glade.getInstance().getSubjectManager().dispatch(hurtcamSubject);
        if (this.mc.getRenderViewEntity() instanceof EntityLivingBase && !hurtcamSubject.isCancelled()) {
    ...
    public void renderHand(float partialTicks, int xOffset) {
        RenderHandSubject renderHandSubject = new RenderHandSubject();
        Glade.getInstance().getSubjectManager().dispatch(renderHandSubject);

        if (renderHandSubject.isCancelled())
            return;
        if (!this.debugView) {
            GlStateManager.matrixMode(5889);
            GlStateManager.loadIdentity();
        ...
        if (this.lightmapUpdateNeeded) {
            ...
            if (var2 != null) {
                ...
                for (int var3 = 0; var3 < 256; ++var3) {
                    ...
                    GammaSubject gammaSubject = new GammaSubject(mc.gameSettings.gammaSetting);
                    Glade.getInstance().getSubjectManager().dispatch(gammaSubject);

                    var16 = gammaSubject.getGamma();
                    var17 = 1.0F - var13;
    ...
    private void renderWorldPass(int pass, float partialTicks, long finishTimeNano) {
        ...
        RenderSubject renderSubject = new RenderSubject(partialTicks);
        GlStateManager.pushMatrix();
        Glade.getInstance().getSubjectManager().dispatch(renderSubject);
        GlStateManager.popMatrix();
