...
public class RenderGuardian extends RenderLiving {
    ...
    public boolean shouldRenderGuardian(EntityGuardian guardian, ICamera camera, double camX, double camY, double camZ) {
        if (super.shouldRenderLiving(guardian, camera, camX, camY, camZ)) {
            return true;
        } else {
            if (guardian.hasTargetedEntity()) {
                ...
                if (var9 != null) {
                    ...
                    Vec3 var11 = this.func_177110_a(guardian, (double) guardian.getEyeHeight(), 1.0F);
    ...
    private Vec3 func_177110_a(EntityLivingBase entityLivingBaseIn, double p_177110_2_, float p_177110_4_) {
        double var5 = entityLivingBaseIn.lastTickPosX + (entityLivingBaseIn.posX - entityLivingBaseIn.lastTickPosX) * (double) p_177110_4_;
        double var7 = p_177110_2_ + entityLivingBaseIn.lastTickPosY + (entityLivingBaseIn.posY - entityLivingBaseIn.lastTickPosY) * (double) p_177110_4_;
        double var9 = entityLivingBaseIn.lastTickPosZ + (entityLivingBaseIn.posZ - entityLivingBaseIn.lastTickPosZ) * (double) p_177110_4_;
        return new Vec3(var5, var7, var9);
    }

    public void renderGuardian(EntityGuardian guardian, double x, double y, double z, float entityYaw, float partialTicks) {
        if (this.field_177115_a != ((ModelGuardian) this.mainModel).func_178706_a()) {
            this.mainModel = new ModelGuardian();
            this.field_177115_a = ((ModelGuardian) this.mainModel).func_178706_a();
        }

        super.doRender((EntityLiving) guardian, x, y, z, entityYaw, partialTicks);
        EntityLivingBase var10 = guardian.getTargetedEntity();

        if (var10 != null) {
            float var11 = guardian.func_175477_p(partialTicks);
            ...
            float var15 = (float) guardian.worldObj.getTotalWorldTime() + partialTicks;
            float var16 = var15 * 0.5F % 1.0F;
            float var17 = guardian.getEyeHeight();
            GlStateManager.pushMatrix();
            GlStateManager.translate((float) x, (float) y + var17, (float) z);
            Vec3 var18 = this.func_177110_a(var10, (double) var10.height * 0.5D, partialTicks);
            Vec3 var19 = this.func_177110_a(guardian, (double) var17, partialTicks);
    ...
    protected void preRenderCallback(EntityGuardian guardian, float partialTicks) {
        if (guardian.isElder()) {
            GlStateManager.scale(2.35F, 2.35F, 2.35F);
        }
    }

    protected ResourceLocation getTextures(EntityGuardian guardian) {
    ...
    public void doRender(EntityLiving entity, double x, double y, double z, float entityYaw, float partialTicks) {
        this.renderGuardian((EntityGuardian) entity, x, y, z, entityYaw, partialTicks);
    }
    ...
    protected ResourceLocation getEntityTexture(Entity entity) {
        return this.getTextures((EntityGuardian) entity);
    }
    ...
    public boolean shouldRender(Entity livingEntity, ICamera camera, double camX, double camY, double camZ) {
        return this.shouldRenderGuardian((EntityGuardian) livingEntity, camera, camX, camY, camZ);
    }
}
