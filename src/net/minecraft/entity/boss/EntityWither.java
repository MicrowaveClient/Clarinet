...
public class EntityWither extends EntityMob implements IBossDisplayData, IRangedAttackMob {
    ...
    private static final Predicate attackEntitySelector = new Predicate() {
        public boolean canApply(Entity entityIn) {
            return entityIn instanceof EntityLivingBase && ((EntityLivingBase) entityIn).getCreatureAttribute() != EnumCreatureAttribute.UNDEAD;
        }

        public boolean apply(Object p_apply_1_) {
            return this.canApply((Entity) p_apply_1_);
        }
    };
                            ...
                            if (var5 != this && var5.isEntityAlive() && this.canEntityBeSeen(var5)) {
                                if (var5 instanceof EntityPlayer) {
                                    if (!((EntityPlayer) var5).capabilities.disableDamage) {
                                        this.updateWatchedTargetId(var1, var5.getEntityId());
                                    }
                                } else {
                                    this.updateWatchedTargetId(var1, var5.getEntityId());
                                }
