...
public class EntityVillager extends EntityAgeable implements INpc, IMerchant {
    ...
    private void spawnParticles(EnumParticleTypes particleType) {
        for (int var2 = 0; var2 < 5; ++var2) {
            ...
            this.worldObj.spawnParticle(particleType, this.posX + (double) (this.rand.nextFloat() * this.width * 2.0F) - (double) this.width, this.posY + 1.0D + (double) (this.rand.nextFloat() * this.height), this.posZ + (double) (this.rand.nextFloat() * this.width * 2.0F) - (double) this.width, var3, var5, var7, new int[0]);
        }
    }
    ...
    private boolean hasEnoughItems(int multiplier) {
        ...
        for (int var3 = 0; var3 < this.villagerInventory.getSizeInventory(); ++var3) {
            ...
            if (var4 != null) {
                if (var4.getItem() == Items.bread && var4.stackSize >= 3 * multiplier || var4.getItem() == Items.potato && var4.stackSize >= 12 * multiplier || var4.getItem() == Items.carrot && var4.stackSize >= 12 * multiplier) {
                    return true;
                }

                if (var2 && var4.getItem() == Items.wheat && var4.stackSize >= 9 * multiplier) {
                    return true;
                }
            }
        }
    ...
    static class EmeraldForItems implements EntityVillager.ITradeList {
        ...
        public void modifyMerchantRecipeList(MerchantRecipeList recipeList, Random random) {
            ...
            if (this.price != null) {
                var3 = this.price.getPrice(random);
            }

            recipeList.add(new MerchantRecipe(new ItemStack(this.sellItem, var3, 0), Items.emerald));
        }
    }
