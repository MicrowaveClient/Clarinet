...
public class RandomPositionGenerator {
    ...
    private static Vec3 findRandomTargetBlock(EntityCreature entitycreatureIn, int xz, int y, Vec3 targetVec3) {
        ...
        if (entitycreatureIn.hasHome()) {
            double var11 = entitycreatureIn.getHomePosition().distanceSq((double) MathHelper.floor_double(entitycreatureIn.posX), (double) MathHelper.floor_double(entitycreatureIn.posY), (double) MathHelper.floor_double(entitycreatureIn.posZ)) + 4.0D;
            double var13 = (double) (entitycreatureIn.getMaximumHomeDistance() + (float) xz);
        ...
        for (int var17 = 0; var17 < 10; ++var17) {
            ...
            if (targetVec3 == null || (double) var12 * targetVec3.xCoord + (double) var14 * targetVec3.zCoord >= 0.0D) {
                BlockPos var15;

                if (entitycreatureIn.hasHome() && xz > 1) {
                    var15 = entitycreatureIn.getHomePosition();

                    if (entitycreatureIn.posX > (double) var15.getX()) {
                        var12 -= var4.nextInt(xz / 2);
                    } else {
                        var12 += var4.nextInt(xz / 2);
                    }

                    if (entitycreatureIn.posZ > (double) var15.getZ()) {
