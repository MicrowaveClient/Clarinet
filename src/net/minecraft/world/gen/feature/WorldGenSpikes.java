        ...
        if (worldIn.isAirBlock(position) && worldIn.getBlockState(position.down()).getBlock() == this.baseBlockRequired) {
            ...
            for (var6 = position.getX() - var5; var6 <= position.getX() + var5; ++var6) {
                for (var7 = position.getZ() - var5; var7 <= position.getZ() + var5; ++var7) {
                    ...
                    if (var8 * var8 + var9 * var9 <= var5 * var5 + 1 && worldIn.getBlockState(new BlockPos(var6, position.getY() - 1, var7)).getBlock() != this.baseBlockRequired) {
                        return false;
                    }
                }
            }
            ...
            var11.setLocationAndAngles((double) ((float) position.getX() + 0.5F), (double) (position.getY() + var4), (double) ((float) position.getZ() + 0.5F), rand.nextFloat() * 360.0F, 0.0F);
