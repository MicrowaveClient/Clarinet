package me.nuf.glade.module.impl.render.chat;
...
public final class GuiCustomChat extends GuiNewChat {
    private final CustomFont chatFont = new CustomFont("Verdana Bold", 17);

    public GuiCustomChat(Minecraft minecraft) {
        super(minecraft);
    }
    ...
    public void drawChat(int p_146230_1_) {
        if (this.mc.gameSettings.chatVisibility != EntityPlayer.EnumChatVisibility.HIDDEN) {
            ...
            if (var5 > 0) {
                ...
                GlStateManager.scale(var7, var7, 1F);
                int var9;
                int var11;
                int var14;
                ...
                int height = getChatOpen() ? -var2 * 9 : -var4 * 9;
                float width = getChatWidth();
                GlStateManager.enableBlend();
               /* if (getChatOpen()) {
                    RenderMethods.drawBorderedRectReliant(1F, -var2 * 9 - 17, width + 15F, -var2 * 9 - 3, 1.7F, 0x66000000, 0x33000000);
                    chatFont.drawString("Chat", 4F, -var2 * 9 - 18.5F, CustomFont.FontType.SHADOW_THIN, 0xFFFFFFFF);
                    RenderMethods.drawBorderedRectReliant(1F, height - 1, width + 15F, 2.5F, 1.7F, 0x66000000, 0x33000000);
                } else*/
                if (var4 > 0) {
                    RenderMethods.drawBorderedRectReliant(1F, height - 1, width + 15F, 2.5F, 1.7F, 0x66000000, 0x33000000);
                }
                ...
                for (var9 = 0; var9 + this.scrollPos < this.field_146253_i.size() && var9 < var2; ++var9) {
                    ...
                    if (var10 != null) {
                        ...
                        if (var11 < 200 || var3) {
                            double var12 = (double) var11 / 200D;
                            var12 = 1D - var12;
                            var12 *= 10D;
                            var12 = MathHelper.clamp_double(var12, 0D, 1D);
                            var12 *= var12;
                            ...
                            if (var14 > 3) {
                                ...
                                GlStateManager.enableBlend();
                                ChatMessageSubject chatMessageSubject = new ChatMessageSubject(var17);
                                Glade.getInstance().getSubjectManager().dispatch(chatMessageSubject);
                                if (chatMessageSubject.isCancelled())
                                    chatMessageSubject.setText(Glade.getInstance().getFriendManager().replace(chatMessageSubject.getText()));
                                chatFont.drawString(chatMessageSubject.getText(), (float) var15 + 3, (var16 - 12F), CustomFont.FontType.SHADOW_THIN, 0xFFFFFFFF);
    ...
    public void addToSentMessages(String p_146239_1_) {
        if (this.sentMessages.isEmpty()
                || !(this.sentMessages.get(this.sentMessages.size() - 1)).equals(p_146239_1_))
    ...
    public IChatComponent getChatComponent(int p_146236_1_, int p_146236_2_) {
        if (!this.getChatOpen()) {
            ...
            if (var6 >= 0 && var7 >= 0) {
                ...
                if (var6 <= MathHelper.floor_float((float) this.getChatWidth() / this.getChatScale())
